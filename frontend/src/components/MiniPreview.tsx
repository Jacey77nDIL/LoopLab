...
export default function MiniPreview({ code }: { code: string }) {
  ...
  useEffect(() => {
    ...
    // Make iframe backgrounds obey light/dark theme variables natively
...
<body>
  ...
  if (!code) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-[var(--color-arcade-panel)] border-2 border-dashed border-[var(--color-arcade-border)] w-full h-full p-4 relative transition-colors">
  ...
  return (
    <div className="flex-1 flex flex-col bg-[var(--color-arcade-panel)] border-crt shadow-neon-cyan relative p-2 overflow-hidden w-full h-full transition-colors">
      <div className="absolute top-0 left-0 w-full h-1 bg-[var(--color-arcade-cyan)] opacity-70 z-10"></div>

      <div className="flex-1 relative bg-[var(--color-arcade-iframe-bg)] border border-[var(--color-arcade-border)] shadow-inner transition-colors">
        <iframe
          ref={iframeRef}
      ...
      <div className="mt-2 flex justify-between items-center text-[10px] text-[var(--color-arcade-text)] font-mono uppercase tracking-widest px-2">
        ...
        <button
          ...
          className="hover:text-[var(--color-arcade-cyan)] hover:bg-[var(--color-arcade-bubble-user)] text-[var(--color-arcade-text)] transition-colors border border-[var(--color-arcade-border)] px-2 py-1"
        >
          &gt; FLUSH_CACHE
        </button>
      </div>
